...
<head>
<meta charset="UTF-8">
<title>uBuy View my Auctions</title>
...
<body>
	...
	<div align="center">
		<h2>My Auctions:</h2>
		<h3>Please click on the AuctionId Hyperlink to View the
			respective Auction</h3>
		<br>

		<%
			AuctionManagement am = new AuctionManagement();
			ArrayList<Auction> all = am.getAllAuctions();
		%>
		...
		<table class="table table-striped">
			...
			<%
				for (int i = 0; i < all.size(); i++) {
					if (all.get(i).getCreatedByUserId().equals(user.getId())) {
			%>

			<tr>
				<td><div align="center">
						<h3>
							<a href="ViewThisAuctionServlet?param1=<%=all.get(i).getId()%>">
								View this auction <%
								// =all.get(i).getId()
							%>
							</a>
				...
				<td><div align="center">
						<h3><%=all.get(i).getTitle()%></h3>
					</div></td>
				<td><div align="center">
						<h3><%=!all.get(i).isExpired()%></h3>
					</div></td>
				<td><div align="center">
						<h3><%=all.get(i).getStartPrice()%></h3>
					</div></td>
				<td><div align="center">
						<h3><%=am.getHighestBidAmount(all.get(i))%></h3>
					</div></td>
				<td><div align="center">
						<h3><%=all.get(i).getAuctionGroup()%></h3>
					</div></td>
				<td><div align="center">
						<h3><%=am.auctionDaysLeft(all.get(i))%></h3>
					</div></td>
			</tr>
			<%
				}
